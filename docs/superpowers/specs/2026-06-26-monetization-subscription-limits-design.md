...
- **Limit-reached behavior:** block the *next* action (a dictation already in progress finishes and is injected; the next one is blocked). Meetings block before the 3rd recording starts. Both surface an upgrade prompt.
...
1. **Metering + limits + account gate** — `usage_events`, weekly usage, the cache-first enforcement (Rust + React), the onboarding login gate, the usage indicator, and the upgrade modal *shell* (button can be inert until phase 2). At the end of phase 1, free limits are enforced and everyone is on `free`.
