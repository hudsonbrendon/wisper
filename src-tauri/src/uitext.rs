...
const EN: Dict = &[
    ...
    ("tray_quit", "Quit OpenWispr"),
    ("err_no_audio", "No audio captured — hold the hotkey while you speak."),
    ("err_no_mic_mac", "No audio detected — grant OpenWispr's Microphone permission in System Settings → Privacy & Security → Microphone, then try again."),
    ...
    ("err_accessibility", "Grant Accessibility permission to OpenWispr (System Settings → Privacy & Security → Accessibility). If OpenWispr is already listed and enabled, toggle it off and on — the previous build's permission goes stale after an update."),
];
...
const PT: Dict = &[
    ...
    ("tray_quit", "Sair do OpenWispr"),
    ("err_no_audio", "Nenhum áudio captado — segure o atalho enquanto fala."),
    ("err_no_mic_mac", "Nenhum áudio detectado — conceda a permissão de Microfone ao OpenWispr em Ajustes do Sistema → Privacidade e Segurança → Microfone, e tente de novo."),
    ...
    ("err_accessibility", "Conceda a permissão de Acessibilidade ao OpenWispr (Ajustes do Sistema → Privacidade e Segurança → Acessibilidade). Se o OpenWispr já estiver listado e ativado, desligue e ligue de novo — a permissão da versão anterior fica obsoleta após uma atualização."),
];
...
const ES: Dict = &[
    ...
    ("tray_quit", "Salir de OpenWispr"),
    ("err_no_audio", "No se capturó audio — mantén pulsada la tecla de acceso rápido mientras hablas."),
    ("err_no_mic_mac", "No se detectó audio — concede el permiso de Micrófono a OpenWispr en Ajustes del Sistema → Privacidad y seguridad → Micrófono y vuelve a intentarlo."),
    ...
    ("err_accessibility", "Concede el permiso de Accesibilidad a OpenWispr (Ajustes del Sistema → Privacidad y seguridad → Accesibilidad). Si OpenWispr ya aparece en la lista y está habilitado, desactívalo y vuelve a activarlo — el permiso de la compilación anterior caduca tras una actualización."),
];
const FR: Dict = &[
    ("tray_home", "Accueil"),
    ...
    ("tray_quit", "Quitter OpenWispr"),
    ("err_no_audio", "Aucun audio capturé — maintenez le raccourci enfoncé pendant que vous parlez."),
    ("err_no_mic_mac", "Aucun audio détecté — accordez l'autorisation Microphone à OpenWispr dans Réglages Système → Confidentialité et sécurité → Microphone, puis réessayez."),
    ...
    ("err_accessibility", "Accordez l'autorisation Accessibilité à OpenWispr (Réglages Système → Confidentialité et sécurité → Accessibilité). Si OpenWispr est déjà dans la liste et activé, désactivez-le puis réactivez-le — l'autorisation de la version précédente devient obsolète après une mise à jour."),
];
const IT: Dict = &[
    ("tray_home", "Home"),
    ...
    ("tray_quit", "Esci da OpenWispr"),
    ("err_no_audio", "Nessun audio acquisito — tieni premuto il tasto di scelta rapida mentre parli."),
    ("err_no_mic_mac", "Nessun audio rilevato — concedi il permesso Microfono a OpenWispr in Impostazioni di Sistema → Privacy e sicurezza → Microfono, quindi riprova."),
    ...
    ("err_accessibility", "Concedi il permesso Accessibilità a OpenWispr (Impostazioni di Sistema → Privacy e sicurezza → Accessibilità). Se OpenWispr è già nell'elenco e abilitato, disabilitalo e riabilitalo — il permesso della versione precedente diventa obsoleto dopo un aggiornamento."),
];
const DE: Dict = &[
    ...
    ("tray_quit", "OpenWispr beenden"),
    ("err_no_audio", "Kein Audio aufgenommen — halte den Hotkey gedrückt, während du sprichst."),
    ("err_no_mic_mac", "Kein Audio erkannt — erteile OpenWispr die Mikrofonberechtigung unter Systemeinstellungen → Datenschutz & Sicherheit → Mikrofon und versuche es erneut."),
    ...
    ("err_accessibility", "Erteile OpenWispr die Bedienungshilfenberechtigung (Systemeinstellungen → Datenschutz & Sicherheit → Bedienungshilfen). Wenn OpenWispr dort bereits aktiviert ist, deaktiviere und reaktiviere es — die Berechtigung des vorherigen Builds wird nach einem Update ungültig."),
];
const NL: Dict = &[
    ...
    ("tray_quit", "OpenWispr afsluiten"),
    ("err_no_audio", "Geen audio opgenomen — houd de sneltoets ingedrukt terwijl je spreekt."),
    ("err_no_mic_mac", "Geen audio gedetecteerd — geef OpenWispr toestemming voor de microfoon via Systeeminstellingen → Privacy en beveiliging → Microfoon en probeer het opnieuw."),
    ...
    ("err_accessibility", "Geef OpenWispr toegangsbeheer-toestemming (Systeeminstellingen → Privacy en beveiliging → Toegankelijkheid). Als OpenWispr al in de lijst staat en is ingeschakeld, schakel het dan uit en weer in — de toestemming van een vorige versie verloopt na een update."),
];
const PL: Dict = &[
    ...
    ("tray_quit", "Zamknij OpenWispr"),
    ("err_no_audio", "Nie zarejestrowano dźwięku — przytrzymaj skrót klawiszowy podczas mówienia."),
    ("err_no_mic_mac", "Nie wykryto dźwięku — przyznaj OpenWispr dostęp do mikrofonu w Ustawienia systemowe → Prywatność i bezpieczeństwo → Mikrofon, a następnie spróbuj ponownie."),
    ...
    ("err_accessibility", "Przyznaj OpenWispr uprawnienie dostępu ułatwień dostępu (Ustawienia systemowe → Prywatność i bezpieczeństwo → Dostępność). Jeśli OpenWispr jest już na liście i włączony, wyłącz go i włącz ponownie — uprawnienie poprzedniej wersji wygasa po aktualizacji."),
];
const RU: Dict = &[
    ("tray_home", "Главная"),
    ...
    ("tray_quit", "Выйти из OpenWispr"),
    ("err_no_audio", "Аудио не записано — удерживайте горячую клавишу во время речи."),
    ("err_no_mic_mac", "Аудио не обнаружено — разрешите OpenWispr доступ к микрофону в «Системных настройках» → «Конфиденциальность и безопасность» → «Микрофон», затем повторите попытку."),
    ...
    ("err_accessibility", "Предоставьте OpenWispr разрешение на доступ к специальным возможностям («Системные настройки» → «Конфиденциальность и безопасность» → «Универсальный доступ»). Если OpenWispr уже есть в списке и включён, снимите и снова установите флажок — разрешение предыдущей версии устаревает после обновления."),
];
const TR: Dict = &[
    ("tray_home", "Ana Sayfa"),
    ...
    ("tray_quit", "OpenWispr'dan Çık"),
    ("err_no_audio", "Ses kaydedilemedi — konuşurken kısayol tuşunu basılı tutun."),
    ("err_no_mic_mac", "Ses algılanamadı — OpenWispr için Sistem Ayarları → Gizlilik ve Güvenlik → Mikrofon yolundan mikrofon iznini verin, ardından tekrar deneyin."),
    ...
    ("err_accessibility", "OpenWispr'a Erişilebilirlik izni verin (Sistem Ayarları → Gizlilik ve Güvenlik → Erişilebilirlik). OpenWispr zaten listede ve etkinse, izni kapatıp yeniden açın — önceki sürümün izni güncelleme sonrasında geçersiz hale gelir."),
];
const AR: Dict = &[
    ("tray_home", "الرئيسية"),
    ...
    ("tray_quit", "إنهاء OpenWispr"),
    ("err_no_audio", "لم يُسجَّل أي صوت — اضغط مع الاستمرار على المفتاح المختصر أثناء حديثك."),
    ("err_no_mic_mac", "لم يُكتشَف أي صوت — امنح OpenWispr إذن الوصول إلى الميكروفون من إعدادات النظام → الخصوصية والأمان → الميكروفون، ثم حاول مجدداً."),
    ...
    ("err_accessibility", "امنح OpenWispr إذن الوصول إلى الإمكانية (إعدادات النظام → الخصوصية والأمان → الإمكانية). إذا كان OpenWispr مدرجاً ومُفعَّلاً بالفعل، فأوقف تشغيله ثم أعد تفعيله — إذ يصبح إذن الإصدار السابق غير صالح بعد التحديث."),
];
const JA: Dict = &[
    ("tray_home", "ホーム"),
    ...
    ("tray_quit", "OpenWispr を終了"),
    ("err_no_audio", "音声が録音されませんでした — ホットキーを押しながら話してください。"),
    ("err_no_mic_mac", "音声が検出されませんでした — システム設定 → プライバシーとセキュリティ → マイク で OpenWispr にマイクのアクセス許可を付与してから、もう一度お試しください。"),
    ...
    ("err_accessibility", "OpenWispr にアクセシビリティの許可を付与してください（システム設定 → プライバシーとセキュリティ → アクセシビリティ）。すでにリストに表示されて有効になっている場合は、一度オフにしてからオンに切り替えてください — アップデート後は以前のビルドのアクセス許可が無効になります。"),
];
const KO: Dict = &[
    ("tray_home", "홈"),
    ...
    ("tray_quit", "OpenWispr 종료"),
    ("err_no_audio", "오디오가 녹음되지 않았습니다 — 말하는 동안 단축키를 누르고 계세요."),
    ("err_no_mic_mac", "오디오가 감지되지 않았습니다 — 시스템 설정 → 개인 정보 보호 및 보안 → 마이크 에서 OpenWispr의 마이크 권한을 허용한 후 다시 시도하세요."),
    ...
    ("err_accessibility", "OpenWispr에 손쉬운 사용 권한을 부여하세요（시스템 설정 → 개인 정보 보호 및 보안 → 손쉬운 사용）. OpenWispr이 이미 목록에 있고 활성화되어 있다면 껐다가 다시 켜세요 — 업데이트 후에는 이전 빌드의 권한이 만료됩니다."),
];
const ZH: Dict = &[
    ("tray_home", "主页"),
    ("tray_updates", "检查更新"),
    ...
    ("tray_quit", "退出 OpenWispr"),
    ("err_no_audio", "未录制到音频 — 请在说话时按住快捷键。"),
    ("err_no_mic_mac", "未检测到音频 — 请在系统设置 → 隐私与安全性 → 麦克风 中授予 OpenWispr 麦克风权限，然后重试。"),
    ...
    ("err_accessibility", "请授予 OpenWispr 辅助功能权限（系统设置 → 隐私与安全性 → 辅助功能）。如果 OpenWispr 已在列表中并已启用，请先关闭再重新开启 — 更新后旧版本的权限会失效。"),
];
const HI: Dict = &[
    ("tray_home", "होम"),
    ...
    ("tray_quit", "OpenWispr बंद करें"),
    ("err_no_audio", "कोई ऑडियो रिकॉर्ड नहीं हुआ — बोलते समय हॉटकी दबाए रखें।"),
    ("err_no_mic_mac", "कोई ऑडियो नहीं मिला — सिस्टम सेटिंग्स → गोपनीयता और सुरक्षा → माइक्रोफ़ोन में OpenWispr को माइक्रोफ़ोन की अनुमति दें, फिर दोबारा प्रयास करें।"),
    ...
    ("err_accessibility", "OpenWispr को एक्सेसिबिलिटी की अनुमति दें (सिस्टम सेटिंग्स → गोपनीयता और सुरक्षा → एक्सेसिबिलिटी)। अगर OpenWispr पहले से सूची में है और सक्षम है, तो उसे बंद करके फिर चालू करें — अपडेट के बाद पुराने बिल्ड की अनुमति अमान्य हो जाती है।"),
];
...
mod tests {
    ...
    fn falls_back_to_english_for_unknown_lang() {
        assert_eq!(t("xx", "tray_quit"), "Quit OpenWispr");
    }
