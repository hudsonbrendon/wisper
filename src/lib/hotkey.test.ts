...
describe("eventToAccelerator", () => {
  it("builds a modifier+key combo", () => {
    expect(
      eventToAccelerator(
        ev({ ctrlKey: true, shiftKey: true, code: "KeyZ", key: "z" }),
      ),
  ...
  it("maps Super for the command key and named keys", () => {
    expect(
      eventToAccelerator(ev({ metaKey: true, code: "Space", key: " " })),
    ).toBe("Super+Space");
  ...
  it("returns null while only modifiers are held", () => {
    expect(
      eventToAccelerator(ev({ altKey: true, key: "Alt", code: "AltLeft" })),
    ).toBeNull();
    expect(
      eventToAccelerator(ev({ ctrlKey: true, shiftKey: true })),
    ).toBeNull();
  });
});
...
describe("loneModifierFromKeyup", () => {
  ...
  it("returns null when another modifier is still held", () => {
    expect(
      loneModifierFromKeyup(ev({ key: "Shift", ctrlKey: true })),
    ).toBeNull();
  });
